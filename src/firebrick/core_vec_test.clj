...
(defn crappy [p1 p2] (class p1))


(defprotocol vec-proto
  (vp+ [v1 v2] [v1 v2 v3]))


(extend-protocol vec-proto Vec2f
  (vp+ ([^Vec2f v1 ^Vec2f v2] (Vec2f/sum v1 v2))
       ([^Vec2f v1 ^Vec2f v2 ^Vec2f v3] (Vec2f/sum v1 v2 v3))))

(vp+ (Vec2f. 1 3) (Vec2f. 3 4) (Vec2f. 1 1))
...
(defn v7+ [& vecs]
  (reduce v+ (Vec2f. 0 0) vecs))

(defn v8+ [& vecs]
  (reduce vp+ (Vec2f. 0 0) vecs))
...
(time (dotimes [i 10000] (apply v7+ tmp2)))
(time (dotimes [i 10000] (apply v8+ tmp2))))
